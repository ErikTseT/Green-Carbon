	...
	echo '<center>
			...
			<center>
				<div class="mo-oauth-test-successed" hidden><div style="display: grid; grid-template-columns: 5fr 1fr; grid-gap: 20px;">
					...
					<div><input type="submit" class="mo-button mo-oauth-next-setup mo-btn-test-finish_class" value="Finish"  id="mo-btn-test-finish"></div></div>
					...
					for(var i=1; i<length;i++){
						for(var key in display_log_arr){	
							...
							if(mo_oauth_test_ajax_count > ajaxLimit){
							    if(-1 === logs[i][0].indexOf(key) && key == "Token Request Sent" && currentStep== "6" ){
                                    mooauth_get_suggestion_troubleshooting("timeExceed");
									jQuery("#mo-btn-test-re-run").show();

							    }
							}
							 if((undefined !== logs[i][1] && (-1 !== logs[i][1].indexOf("ERROR") || -1 !== logs[i][1].indexOf("error") )) && key == "Token Response Received" && currentStep== "6"){
