...
class Co2ok_AdminOverview
{
    ...
    {
      $returnValue = '';
      if (in_array($setting, ['co2ok_optout', 'co2ok_cfp', 'co2ok_gif_feature', 'co2ok_ab_research', 'co2ok_widgetmark_footer'], true)) {
        ...
        if (in_array($input, ['after_order_notes', 'before_checkout_form', 'checkout_before_customer_details', 'after_checkout_billing_form', 'checkout_order_review'], true)) {
          $returnValue = $input;
        } else {
          $returnValue = 'checkout_order_review';
        }
      } else if ($setting == 'co2ok_button_template') {
        if (in_array($input, ['co2ok_button_template_default', 'co2ok_button_template_minimal'])) {
          $returnValue = $input;
        } else {
          $returnValue = 'co2ok_button_template_default';
        }
      } else if ($setting == 'co2ok_disable_button_on_cart') {
    ...
    function co2ok_plugin_admin_overview()
    {
        ...
        {
            update_option('co2ok_cfp', $this->validate_setting_input('co2ok_cfp', sanitize_key($_POST['co2ok_cfp'])));
        }
        ...
        {
            update_option('co2ok_widgetmark_footer', $this->validate_setting_input('co2ok_widgetmark_footer', sanitize_key($_POST['co2ok_widgetmark_footer'])));
        }
        ...
        {
            update_option('co2ok_disable_button_on_cart', $this->validate_setting_input('co2ok_disable_button_on_cart', sanitize_key($_GET['co2ok_disable_button_on_cart'])));
        }
