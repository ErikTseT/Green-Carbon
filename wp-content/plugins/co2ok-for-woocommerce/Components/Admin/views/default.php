<?php
    ...
    if (isset($_GET['byjensen_process_orders'])){
        global $woocommerce;

        // get the orders since accounting broke
        $args = array(
            'date_created' => '2021-11-01...2022-05-23',
            'orderby' => 'date',
            'order' => 'DESC',
            );
        ...
        foreach ($orders as $order) {
            ...
            foreach ($fees as $fee) { 
                if (stripos ($fee->get_name(), 'CO2' ) !== false) {
                    \co2ok_plugin_woocommerce\Co2ok_Plugin::co2ok_storeTransaction($order->get_id());
                }
            }
        }
    }
