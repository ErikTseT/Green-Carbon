...
if ( !class_exists( 'co2ok_plugin_woocommerce\Co2ok_Plugin' ) ) :
    ...
    final static function co2ok_storeTransaction($order_id)
    {
        $order = wc_get_order($order_id);
        $fees = $order->get_fees();
        ...
        $compensationCost = 0;
        foreach ($fees as $fee) {
            if (stripos ($fee->get_name(), 'CO2' ) !== false) {
    ...
    {
        /**
         * Retrieves total impact this shop had in the fight against climate change
         * Estimates number of compensations until new api is finalised
         * Result is stored in amount of compensations and kg of CO2 compensated
         */

        $merchantCode = get_option('co2ok_code');
        $url = "https://app.co2ok.eco/user/totalCompensationData/?merchantId=" . $merchantCode;

        $http = _wp_http_get_object();
    
        // retrieve kg of CO2 compensated
        $response = $http->get($url);
        $impactTotal = $response['body'];
    
        // Estimate number of compensations based on average compensation
        $compensationCount = intval($impactTotal / 35);
    
        // Don't show zero for shops who've just installed us
