...
> If there is a need to release this now, please remove the step where it commits to svn and see if everything goes correct first.
> For instance, there would be a bug now because it overwrites version numbers, but does not ignore the one under Changelog yet!

Manual method can be found here: https://devazure.com/CO2ok/Development/_wiki/wikis/Development.wiki/34/WooCommerce-Deployment.

Make sure you have `rsync` installed, this should only be run on a Linux based environment (WSL should be fine).

1. Add a new entry under the Changelog section of `readme.txt`.
2. Remember what was the previous version and the version you are releasing.
3. Run `make OLDVERSION=x.x.x.x NEWVERSION=x.x.x.x release`

This will push the code to the Wordpress SVN repositories.

You can confirm this here: https://plugins.trac.wordpress.org/timeline

That timeline is shared with all Wordpress developers though, so if you are quick enough your release will still be somewhere at the top.
